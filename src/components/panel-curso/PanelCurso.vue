  ...
  <div class="panel-layout">
    <nav class="navbar navbar-dark bg-dark px-3 shadow-sm mb-4">
      <div class="container-fluid d-flex justify-content-between align-items-center">
        <span class="navbar-brand fw-bold text-truncate fs-6 fs-md-5">Panel Curso AAAB</span>
    ...
    <div class="container-fluid px-3 px-md-4 py-2">
      <div class="mx-auto">
        ...
        <div class="user-header d-flex align-items-center justify-content-between mb-4 p-2 p-md-3 rounded-4 shadow">
          <div class="d-flex align-items-center">
            ...
            <div>
              ...
              <span class="badge bg-danger mt-1 px-2 py-1 shadow-sm">ID Aspirante: {{ aspirante.id }}</span>
            </div>
          </div>
          ...
          <div class="dropdown me-1 me-md-2">
            ...
            <ul class="dropdown-menu dropdown-menu-end shadow-lg py-0 overflow-hidden dropdown-notificaciones" style="max-width: 95vw;">
              ...
              <li class="p-2 text-center border-top bg-light d-flex flex-column gap-2">
                ...
                <RouterLink to="/panel-curso/historial-notificaciones" class="btn btn-link btn-sm text-decoration-none text-primary small fw-bold p-0">
        ...
        <div v-if="route.name !== 'PanelInicioCurso'" class="mb-4">
...
// Datos del Aspirante
const aspirante = ref(auth.getUser() || {});

const urlFoto = computed(() => {
  if (!aspirante.value || !aspirante.value.dni) {
    return 'https://ui-avatars.com/api/?name=Sin+Foto&background=ef4444&color=fff';
  }
  const dniLimpio = String(aspirante.value.dni).trim();
  // Se mantiene la misma ruta por defecto, ajustala si las fotos del curso van a otra carpeta
  return `https://arbitroshandball.com.ar/uploads/carnet-arbitros/${dniLimpio}.webp`;
});

const nombreFormateado = computed(() => {
  const nombre = aspirante.value.nombre || '';
...
useHead({
  title: 'Panel de Curso | AAAB',
  meta: [
    { name: 'description', content: 'Panel de control oficial para el curso de aspirantes de la AAAB.' },
    { property: 'og:title', content: 'Panel de Curso | AAAB' },

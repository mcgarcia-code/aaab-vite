              ...
              <table class="table table-hover align-middle mb-0 text-nowrap tabla-fija" style="font-size: 0.75rem; table-layout: fixed;">
                <thead class="table-light">
                  <tr>
                    ...
                    <th class="py-3 text-center text-uppercase text-muted col-fija col-acciones" style="width: 160px;">Acciones</th>
    ...
    <ModalBase :show="mostrarEdicion" @close="cerrarEdicion" titulo="Editar informe" icono="edit" colorIcono="bg-primary text-white" maxWidth="640px">
      <div v-if="informeEdit" class="text-start">
        ...
        <div class="row g-2 mb-3">
          ...
          <div class="col-12" v-if="informeEdit.delegado_tecnico">
            <label class="form-label small fw-bold text-dark mb-1">Delegado técnico (cargó el informe)</label>
            <input type="text" class="form-control form-control-sm bg-light input-readonly" :value="informeEdit.delegado_tecnico" readonly>
          </div>
        </div>
        ...
        <div class="mb-1" v-if="puedeEditar(informeEdit)">
          <label class="form-label small fw-bold text-dark mb-1">
...
// es_autor lo marca el backend: true si el usuario cargó el informe.
...
const puedeAnular = (inf) => esAutor(inf) && inf.estado === 'creado';
...
const guardarEdicion = async () => {
  ...
  try {
    ...
    if (res.ok || res.success) {
      ...
      // Reflejar cambios de archivos: el backend devuelve la lista final
...
@media (min-width: 768px) {
  ...
  .col-acciones { left: 50px; min-width: 160px !important; max-width: 160px !important; }
  .col-fecha    {
    left: 210px;
